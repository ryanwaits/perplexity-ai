...
export const Search: FC<SearchProps> = ({
  ...
  const handleSave = () => {
    ...
    localStorage.setItem("PERPLEXITY_KEY", apiKey);
  ...
  const handleClear = () => {
    localStorage.removeItem("PERPLEXITY_KEY");

    setApiKey("");
  };

  useEffect(() => {
    const PERPLEXITY_KEY = localStorage.getItem("PERPLEXITY_KEY");

    if (PERPLEXITY_KEY) {
      setApiKey(PERPLEXITY_KEY);
    } else {
      ...
      {loading ? (
        ...
        <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center space-y-6 px-3 pt-32 sm:pt-64">
          <div className="flex items-center">
            <IconBolt size={36} />
            <div className="ml-1 text-center text-4xl">PERPLEXITY</div>
          </div>
